...
This has noting to do with scale(yet), and everything to do with managing code complicity and
...
The pattern I have described in this post relies heavily on the use of the enable option, and I think
there is a value to expand a bit on how they are used for this pattern(beyond the context of the pattern )
and use this post to expand a bit about non enable options and how they streamline the configuration
even more.

## Taken the enable option a step further
The pattern that is described in this post is implemented by enabling low level options,
but there is no need to stop at the high level evaluation of the configuration, and once
everything is wrapped inside of an enable option, it can be taken a step further by using it
in other low level options as well. This way by enabling a single option, multiple other
configurations get enabled. For example by setting deluge.enable = true; Other services
such as nginx, postgresql, adguardhome, gatus... are reevaluated and immediately enable
the relevant configurations snippets for deluge. And same is true for setting deluge.enable = false;
A single line controls multiple low level configurations.

### Appending to lists with optionals
...
### Extending attribute sets with optionalAttrs
  ...
  virtualHosts."${hostIP}" = {
    ...
    locations = lib.recursiveUpdate { } (
      lib.optionalAttrs config.services.syncthing.enable {
        "/syncthing/" = {
          proxyPass = "${localHost}:${toString config.custom.services.syncthing.httpPort}/";
        };
      }
      // lib.optionalAttrs config.services.deluge.enable {
        "/deluge/" = {
          proxyPass = "${localHost}:${toString config.services.deluge.web.port}/";
        };
      }
      // lib.optionalAttrs config.services.jellyfin.enable {
        "/jellyfin" = {
          proxyPass = "${localHost}:${toString config.custom.servicePort.jellyfin}";
        };
      }
    );
  };
...
### Adding to strings with optionalString

```nix
services.myService = {
  enable = true;

  serviceConfig.ExecStart = pkgs.writeShellScript "run-my-service" ''
    echo "Starting my service..."

    ${lib.optionalString config.services.myService.debug ''
